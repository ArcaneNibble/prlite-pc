/*  pi_teleop.js - Version 1.0 2012-09-22
...
var pubHandle = null;
var stopHandle = null;
var cameraInfo = "/kinect/rgb/camera_info";
var roiTopic = "/roi";
...
var minAngularSpeed = 1.2;
var vx_key_increment = 0.01;
var vz_key_increment = 0.01;
var deadZoneVz = 0.2;
var vx = 0;
var vz = 0;
var lastVx = 0;
...
// Laser scan topics
var baseLaserScanTopic = "/base_scan"
...
var headPanPositionTopic = "/head_pan_controller/command";
var headTiltPositionTopic = "/head_tilt_controller/command";
...
var headPanSpeedService = '/head_pan_controller/set_speed';
var headTiltSpeedService = '/head_tilt_controller/set_speed';
...
var headPanTorqueService = "/head_pan_controller/torque_enable";
var headTiltTorqueService = "/head_tilt_controller/torque_enable";

// var headPanTempTopic = "/head_pan_joint/state";
// var headTiltTempTopic = "/head_tilt_joint/state";
var headPanTempTopic = "/head_pan_controller/state";
var headTiltTempTopic = "/head_tilt_controller/state";
...
var robotBatteryTopic = "/element/sensor/robot_voltage";
var frontSonarTopic = "/element/sensor/base_sonar_front";
var backSonarTopic = "/element/sensor/base_sonar_back";
var robotBatteryTopic = "/element/sensor/robot_voltage";
...
var safeSonarDistance = 0.50; // meters
var minSonarDistance = 0.03; // meters
var baseSonarFront = safeSonarDistance + 0.1;
var baseSonarBack = safeSonarDistance + 0.1;
var useSonar = false;
...
#    }
    ...
    connection.callService('/rosbridge/get_param', '["pr2lite_teleop"]', function(resp) {
	for (param in resp) {
	...
	if (video2Topic != null) {
	    ...
	    video2Image.src = "http://192.168.1.14/mjpeg.cgi";
	}
...
#	    });
	    ...
	    // Otherwise, assume the robot battery topic is /element/sensor/robot_voltage (Pi Robot)
...
#	    });
	...
	connection.addHandler(frontSonarTopic, function(msg) {
	    ...
	    if (useSonar && baseSonarFront < safeSonarDistance && baseSonarFront > minSonarDistance) sonarFrontObstacleDetected = true;
	...
	connection.addHandler(backSonarTopic, function(msg) {
	    ...
	    if (useSonar && baseSonarBack < safeSonarDistance && baseSonarBack > minSonarDistance) sonarBackObstacleDetected = true;
...
#    });
	...
	laserObstacleDetected = msg.data;
    });
	...
	if (!down) {
	    ...
	    //if (vz < 0) vz = 0;
            //vz += vz_key_increment;
	    //if (Math.abs(vx) > vx_key_increment) vx += -sign(vx) * vx_key_increment;
	    vx = 0;
	    if (vz < 0) vz = 0;
...
function toggleSonar() {
    var sonar = document.getElementById("use_sonar");
    if (sonar.checked) {
	sonar.style.checked = false;
	useSonar = true;
    }
    else {
	sonar.style.checked = true;
	useSonar = false;
    }    
}
...
basePadImage.src = "http://" + thisHost + "/images/rovio_ui.png";
log(basePadImage.src);
*/
...
// contextmenu dragend 
basePad.on("touchend mouseup touchleave mouseout dblclick", function() {
    mouseDown = false;
    pubCmdVel(0, 0);
