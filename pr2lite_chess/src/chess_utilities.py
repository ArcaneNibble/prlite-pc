...
class BoardState:
    ...
    def setPiece(self, column, rank, piece):
        ...
        try:
            ...
            print column, rank
            rospy.logdebug("setPiece: invalid row/column")
    ...
    def computeSide(self):
        ...
        for c in 'abcdefgh':
            ...
            rospy.logdebug("Computed side value of: %d" % side)
        if side > 0:
...
class BoardUpdater(threading.Thread):
	...
	# ARD
        self.prob_piece_gone = [0 for i in range(64)]
        self.prob_piece_new = [0 for i in range(64)]
        self.prob_piece_color = [0 for i in range(64)]
        self.prob_piece = [None for i in range(64)]
        self.prob_cnt = 0
	# ARD
        self.start()
    ...
    def callback(self, message):
        ...
        for x_off in [0,-1,1]:
            for y_off in [0,-1,1]:
                ...
                rospy.logdebug("x_off: %s ; y_off: %s" % (x_off,y_off))
                ...
                for piece in message.pieces:
                    ...
                    if not self.board.valid(col, rank):
                        rospy.logdebug("invalid: %s %d" % (col, rank))
                        continue
                ...
                if len(piece_new) + len(piece_color) <= 8:
                  for entry in piece_gone:
                    ...
                    self.prob_piece_gone[offset] = self.prob_piece_gone[offset] + 1 
  		  ...
  		  if self.prob_cnt >= 10 and len(piece_new) + len(piece_color) > 2:
                ...
                if self.board.side == None:
                    ...
                    if len(piece_color) + len(piece_new) == 0 and len(piece_gone) == 0:
                        ...
                        # self.listener.mutex.release() # ARD
                ...
                elif len(piece_color) + len(piece_new) == 0 and len(piece_gone) == 0:
                        rospy.loginfo("no move")
                        self.board.last_move = "none"
                        # self.listener.mutex.release() # ARD
                        return self.setWithOffset(x_off, y_off, temp_board)
...
class GnuChessEngine:
    ...
    def nextMoveGNU(self, move="go", board=None):
        ...
        """
        # get move
        rospy.loginfo("GNU: nextMoveGNU " + move)
        if self.pawning:
            while not rospy.is_shutdown():
                for row in [2,3,4,5]:
