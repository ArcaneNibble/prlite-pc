...
class ChessBoardLocator
{
  public:
    ...
    // static const int square_size = 0.055625;
        ...
        output_image_(true)
    {
        ...
        if (!nh.getParam ("h_rho", h_rho_))
            h_rho_ = 1;
        ROS_DEBUG ("Hough Rho: %d", h_rho_);  
        if (!nh.getParam ("h_threshold", h_threshold_))
            h_threshold_ = 50;
        ROS_DEBUG ("Hough Threshold: %d", h_threshold_); 
        if (!nh.getParam ("h_min_length", h_min_length_))
            h_min_length_ = 100;
        ROS_DEBUG ("Hough Min Length: %d", h_min_length_);    
        if (!nh.getParam ("e_threshold", e_threshold_))
            e_threshold_ = 0.002;
        ROS_DEBUG ("Error Threshold: %f", e_threshold_); 
    ...
    void cameraCallback ( const sensor_msgs::PointCloud2ConstPtr& msg )
    {
        ...
        ROS_DEBUG("Filtered data cloud of size %d", (int)data_filtered.points.size());

        // find centroid of intersections
        Eigen::Vector4f centroid; 
        pcl::compute3DCentroid(data_filtered, centroid);
        ROS_DEBUG("Centroid (%f, %f)", centroid[0], centroid[1]);
        ...
        for( size_t iter = 1; iter < 4; iter++){
            ...
            board.push_back( point(SQUARE_SIZE*iter, SQUARE_SIZE*iter, 0) );     // a1
            board.push_back( point(SQUARE_SIZE*iter, SQUARE_SIZE*(8-iter), 0) ); // a8
            board.push_back( point(SQUARE_SIZE*(8-iter), SQUARE_SIZE*iter, 0) ); // h1
            ...
            for( size_t i = 0; i < a1_candidates.size(); i++ ){
                for( size_t j = 0; j < a8_candidates.size(); j++ ){
                    for( size_t k = 0; k < h1_candidates.size(); k++ ){
                        ...
                        if( (abs(pcl::euclideanDistance(a1,a8) - (SQUARE_SIZE*(8-iter*2))) > 0.02 ) ||
                            (abs(pcl::euclideanDistance(a1,h1) - (SQUARE_SIZE*(8-iter*2))) > 0.02 ) ||
                            (abs(pcl::euclideanDistance(h1,a8) - (SQUARE_SIZE*(8-iter*2)*1.4142)) > 0.02 ) ) continue;
                        // construct a basis
                        ...
                        for( size_t p = 0; p < data_transformed.points.size(); p++)
                        {
                            ...
                            {   
                                for( int y = 0; y <= 8; y++)
                                {
                                    float test = (SQUARE_SIZE*x-pt.x)*(SQUARE_SIZE*x-pt.x)+(SQUARE_SIZE*y-pt.y)*(SQUARE_SIZE*y-pt.y);
                                    if(test < e)
                                        e = test;
                                }
                            }
            ...
            ROS_DEBUG("best score %f", best_score);
            ROS_DEBUG("best points %d", best_points);   
