...
class ChessExecutive:
    def __init__(self):
        ...
        # rospy.loginfo('exec: Waiting for /move_arm')
        ...
        rospy.loginfo('ArmPlanner')
        self.planner = ArmPlanner( listener = self.listener )
        rospy.loginfo('BoardState')

        # subscribe to input
        self.board = BoardState()
        rospy.loginfo('BoardUpdater')
        self.updater = BoardUpdater(self.board, self.listener)
        rospy.loginfo('sub to extract_pieces')
        ...
        rospy.loginfo('Speech Engine')
        self.speech = SpeechEngine()
        rospy.loginfo('Head Engine')
        self.head = HeadEngine()
        rospy.loginfo('set side')
        self.perception_times = list()

        # maybe set side?
        try:
            s = rospy.get_param("side")
            rospy.loginfo("side = " + s) 
            if s == "w" or s == "white":
    ...
    def playGame(self):
        ...
        # are we white/black?
        rospy.loginfo("update board sate")
        self.updateBoardState(True)
        rospy.loginfo("board side")
        ...
        if self.board.side == self.board.BLACK:
            ...
            rospy.loginfo("Ok, I'll play black")
            ...
            rospy.loginfo("Ok, I'll play white. my turn")
            self.board.last_move = "go"

        # loop!
        while not rospy.is_shutdown(): 
            # do move
            rospy.loginfo("playGame main loop ")
            move = self.getMove()
            ...
            if move in castling_extras.keys():
                ...
                rospy.loginfo("Why oh why am I castling?")
            else:
                self.speech.say("Moving my " + self.board.getMoveText(move))
                rospy.loginfo("Moving my " + self.board.getMoveText(move))
            #self.head.look_at_board()
            ...
            if not self.planner.success: 
                ...
                rospy.loginfo("Oh crap! I have failed")
        ...
        while not rospy.is_shutdown():
            ...
            if self.updater.up_to_date:
                if self.board.last_move == "fail":
                    ...
                    if acceptNone:
                        rospy.loginfo("exec: accept none")
                        break
                    else:
                        self.updater.up_to_date = False 
                        rospy.loginfo("exec: Triggering again...")
    ...
    def getMove(self):
	    ...
	    rospy.loginfo("exec: Move " + move)
            if move in castling_extras.keys():
                ...
                if reach > 0.75 and not self.interactive:
                    rospy.loginfo("Move " + m + ", source has reach of " + str(reach) + ", starting to pawn")
                    self.engine.startPawning()
                    return self.engine.nextMoveGNU(self.board.last_move, self.board)        
